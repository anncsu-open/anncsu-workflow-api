...
def test_health():
    resp = client.get("/anncsu/health")
    assert resp.status_code == 200
    assert resp.json() == {"status": "ok"}


def test_workflows_ui_serves_arazzo_ui_page():
    resp = client.get("/anncsu/workflows/ui")
    assert resp.status_code == 200
    ...
    assert "/anncsu/workflows/spec/anncsu-workflow.arazzo.yaml" in body


def test_arazzo_spec_is_served_statically():
    resp = client.get("/anncsu/workflows/spec/anncsu-workflow.arazzo.yaml")
    assert resp.status_code == 200
...
def test_openapi_source_with_special_chars_is_served():
    ...
    resp = client.get(
        "/anncsu/workflows/spec/Specifica API - ANNCSU – Consultazione per le PA.yaml"
    )
    assert resp.status_code == 200
